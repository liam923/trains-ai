...
def test_gameplay_observed(
    ...
    player_actors = {
        player: RandomActor.make(box, player, ObservedState) for player in players
    }
...
def test_gameplay_known(
    ...
    box = box_maker(players)
    player_actors = {
        player: RandomActor.make(box, player, KnownState) for player in players
    }
    game = SimulatedGameActor.make(box)
    ...
    while not game.is_over:
        for p, player in player_actors.items():
            ...
            for p, hand in player.state.hands.items():
                assert hand.destination_cards == game.player_hands[p].destination_cards
                assert (
                    hand.unselected_destination_cards
                    == game.player_hands[p].unselected_destination_cards
                )
                assert hand.train_cards == game.player_hands[p].train_cards
                assert hand.remaining_trains == game.player_hands[p].remaining_trains
                assert hand.remaining_trains == game.player_hands[p].remaining_trains
