...
This is implemented by the function `_probability_of_having_cards_helper` in `trains/util.py`.
...
![Small Board](readme_photos/small_board.png)
...
Below are tables comparing how the different AI algorithms perform against each other. The first shows each AI's win percentage against each AI, and the second show's their average score. These are calculated over 20 games, with AIs splitting going first. Each AI uses `RelativeUf` wrapped around `ImprovedExpectedScoreUf` as its utility function:
